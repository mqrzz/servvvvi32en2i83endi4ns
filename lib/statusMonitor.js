...
async function checkGithubStatus() {
  ...
  try {
    ...
    return { ok: indicator === 'none', error: indicator !== 'none' ? data.status.description : null, latencyMs };
  } catch (err) {
    return { ok: false, error: String(err?.message || err), latencyMs: Date.now() - started };
  }
}
    ...
    if (newStatus !== 'ok') {
      ...
      const { rows: incRows } = await pool.query(
        `INSERT INTO status_incidents (service_id, title, severity, status, created_by)
         VALUES ($1,$2,'major','investigating','auto') RETURNING *`,
        [service.id, `Автоматически обнаружен сбой: ${service.name}`]
      );
      await pool.query(
