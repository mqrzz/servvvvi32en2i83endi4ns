...
const LATENCY_DEGRADED_MS = 5000; // ответил, но дольше 5 сек — "замедление", не полный сбой
const CLEANUP_INTERVAL_MS = 24 * 60 * 60 * 1000; // раз в сутки чистим старую историю проверок
const CHECKS_RETENTION_DAYS = 90; // столько же, сколько показываем на странице — старше не нужно
...
async function checkOne(service) {
  ...
  if (!service.manual_override) {
    // "Ok, но отвечал дольше LATENCY_DEGRADED_MS" — не полный сбой, а замедление
    // (жёлтый). В аптайм это НЕ засчитывается как сбой — сервис был доступен,
    // просто медленно; это только визуальная пометка "есть проблема, не критично".
    let newStatus;
    if (!result.ok) newStatus = result.severity || 'major';
    else if (result.latencyMs && result.latencyMs > LATENCY_DEGRADED_MS) newStatus = 'degraded';
    else newStatus = 'ok';
...
async function handleAutoIncident(service, newStatus, errorDetail) {
  try {
    if (newStatus !== 'ok') {
      ...
      // Разный текст для "реально упал" и "просто отвечает медленно" — раньше оба
      // случая писали "перестала получать успешный ответ", что для медленного, но
      // живого сервиса неправда и вводило в заблуждение.
      const title = newStatus === 'degraded' && !errorDetail
        ? `Замедление в работе: ${service.name}`
        : `Автоматически обнаружен сбой: ${service.name}`;
      const message = newStatus === 'degraded' && !errorDetail
        ? `Автопроверка фиксирует, что «${service.name}» отвечает медленнее обычного. Разбираемся.`
        : `Автопроверка перестала получать успешный ответ от сервиса «${service.name}»${errorDetail ? ` (${errorDetail})` : ''}. Разбираемся.`;
...
// Чистим старую историю проверок раз в сутки — на странице всё равно показываем
// только последние 90 дней, хранить дальше смысла нет, только место занимает.
